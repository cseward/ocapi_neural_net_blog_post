...
[Roland Vollgraf](https://de.linkedin.com/in/rolandvollgraf) and [I](https://de.linkedin.com/in/calvinseward) 
...
to our picker routing problem can be found in polynomial time in the number of aisles. The details 
of the exact algorithm are discussed in [1] and [2] detailing the even simpler case.
...
worker's cart. A worker at the warehouse is no different from a shopper at the supermarket; the slow 
...
Figure 2: S-Shape and optimal pick tours for collecting items from the warehouse. The blue circles 
...
on the left is the so-called S-Shape heuristic, and the figure on the right is the optimal pick tour.

The Batching Problem
--------------------

At Zalando's scale of operations, thousands of new orders are placed every hour, and each order must 
be assigned to a pick list. Once assigned to a pick list, the items in an order are picked, and then 
sent to a worker who packages the orders to be sent out. For our idealized example, we assume that we 
must follow a few rules when splitting orders into pick lists:
...
Figure 3: OCaPi pick tours for ten orders of two items each randomly split over two pick lists. The 
...
collected those, and left ‘v01’ and ‘v02’ for the other pick list.

![optimal pick list split](https://s3-eu-west-1.amazonaws.com/nvidia-blob-post-image/best.png)
Figure 4: Ten orders of two items each split over two pick lists in the best possible way. We see
...
In theory, finding near-optimal splits of the order pool into pick lists should be easy enough: you
just split your orders into pick lists, use the OCaPi algorithm to calculate travel times for all
lists, and use an optimizer with something like [simulated annealing](https://en.wikipedia.org/wiki/Simulated_annealing)
...
10 items through a warehouse zone, and used OCaPi to figure out how long the travel time would be if the
...
The whole point of the exercise was to make the OCaPi travel time estimation faster. So how did we
do? We ran these experiments on a machine with two Intel Xeon E5-2640 and two NVIDIA Tesla K80 accelerators.
We linked Caffe against cuDNN_v2 and OpenBLAS compiled from source. We were able to train one million pick
lists in just 52.6 seconds compute time (see Table 1).  In our case, one million pick lists represented an
epoch, and we had to train many epochs until the test error started to increase again.
...
over some starting (random) split of items between two pick lists for each iteration of the simulated
annealing.

For the setting above and a realistic zone layout, optimized batches allowed the workers to pick on
average 11% more items in a given time, compared to a random batch. Clearly the actual benefit that
one gets depends highly on the order pool, the number of zones, and other factors. What we see here
...
About Calvin Seward
-------------------
Calvin Seward a data scientist for Zalando’s Statistical Learning team, working mainly on computer
...
more about the exciting work being done at Zalando on our [tech blog](https://tech.zalando.com/blog/)
